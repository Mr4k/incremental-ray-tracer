...
let check_collision_with_world ray world t_min t_max =
  ...
  let world = 
    ...
    List.append lst [{ center = { x = 0.0; y = -100.5; z = -1.0 }; radius = 100.0; }]
  ...
  let _materials_list =
    let lst = List.init num_spheres ~f:(fun _ ->
      ...
      (M' { albedo = { x = Random.float 1.0; y = Random.float 1.0; z = Random.float 1.0 }; fuzziness=fuzziness; })) in
...
let resulting_colors =
  time (fun _-> Array.map screen_coords ~f:(fun screen_coord ->
