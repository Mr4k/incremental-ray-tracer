...
let screen_coords = Array.init (screen_width * screen_height) ~f: (
  fun i -> (
    float_of_int (i % screen_width) /. (float_of_int screen_width),
    float_of_int (i / screen_width) /. (float_of_int screen_height)
  ))

let resulting_colors = Array.map screen_coords ~f:(fun screen_coord -> 
  let (x, y) = screen_coord in 
  trace {
    origin = { x = 0.0; y = 0.0; z=0.0; };
    direction = (add lower_left_corner (add (scale horizontal x) (scale vertical y)))
  }
)
...
let () = 
  let packed_color_array = Array.map resulting_colors ~f:(fun color -> pack_color_to_int color) in
  let color_matrix = Array.init screen_height ~f:(fun i -> Array.sub packed_color_array ~pos: (i * screen_width) ~len: screen_width) in
