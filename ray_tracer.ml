...
let check_collision_with_sphere ray sphere t_min t_max = 
  ...
  if Float.(discriminant >. 0.0) then 
    ...
    let temp = match (solution_1_valid, solution_2_valid) with
    ...
    match temp with
    | Some(temp) -> let hit_pos = add (scale ray.direction temp) ray.origin in
      let outward_normal = scale (sub hit_pos sphere.center) (1.0 /. sphere.radius) in
      let face_dir = if Float.(dot ray.direction outward_normal < 0.0) then 1.0 else -.1.0 in
      Some({
        t= temp;
        position= hit_pos;
